...
class ColorizationController:
    ...
    def _load_model(self) -> DynamicUnet:
        ...
        logger.info(f">>> [{self.device}] Model loaded successfully in {time.time() - tic:.4f}s")
        return model
    ...
    def colorize_image(self, img: Image) -> Image:
        ...
        sample = {'L': resized_lab['L'].unsqueeze(0).to(self.device)}
        ...
        model_ab = torch.nn.functional.interpolate(model_ab, size=(height, width), mode='bilinear').cpu()
